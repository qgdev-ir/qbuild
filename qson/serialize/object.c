...
static inline qson_result _write_object_entry(struct qson_serialize_ctx *c, char *key) {
	qson_run(qson_write_string(c, key));
	qson_ctx_write(c, QSON_NAME_SEPARATOR);
	return QSON_RESULT_OK;
}

static inline qson_result _handle_has_next(struct qson_serialize_ctx *c, bool has_next) {
	if (has_next) {
